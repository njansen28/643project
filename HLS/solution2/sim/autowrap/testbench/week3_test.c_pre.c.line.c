#pragma line 1 "C:/Users/Nathaniel2/hls1121/week3_test.c"
...
#pragma line 2 "C:/Users/Nathaniel2/hls1121/week3_test.c" 2
#pragma empty_line
char* genome_file = "../../../long_reference.fasta";
char* read_file = "../../../long_sequences.fastq";
#pragma empty_line
basepair_t ref_genome[1000000 +1];
basepair_t reads[3][100 +1];
#pragma empty_line
orig_t best_fits[3];
#pragma empty_line
void copy_base_pairs(basepair_t *dest, const char *src, int len) {
 ...
 for (i = 0; i < len; i++) {
  switch (src[i]) {
  case 'A': dest[i] = 0; break;
  case 'G': dest[i] = 1; break;
  case 'C': dest[i] = 2; break;
  case 'T': dest[i] = 3; break;
  default:
...
#pragma empty_line
 for (i = 0; i<3; i++) {
  ...
  fgets(read_line, 100*2, f);
  copy_base_pairs(&reads[i][0], &read_line[0], 100);
 }
 fclose(f);
#pragma empty_line
#pragma empty_line
...
#pragma empty_line
 for (i=0; i<1000000/100; i++) {
  fgets(ref_line, 100*2, f);
  if (index + strlen(ref_line) - 1 > 1000000) {
   printf("Input file too big!, i=%d, index=%lld, strlen(ref_line)=%lld\n", i, index, strlen(ref_line) - 1);
   exit(1);
  }
  copy_base_pairs(&ref_genome[i*100], &ref_line[0], 100);
  index = index + 100;
 }
 fclose(f);
}
#pragma line 114 "C:/Users/Nathaniel2/hls1121/week3_test.c"
int main() {
 unsigned int k, i;
 unsigned int max;
 init();
